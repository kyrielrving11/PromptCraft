#!/usr/bin/env python3
"""Render a zero-shot chain-of-thought prompt from JSON input."""
...
DEFAULT_ROLE = "你是一名严谨的逐步推理助手。"
DEFAULT_OUTPUT_FORMAT = "先给出简明推理过程，再给出最终答案。"
DEFAULT_TARGET_INPUT = "等待用户提供待处理内容。"
DEFAULT_CONSTRAINTS = "不要添加与任务无关的解释。"
DEFAULT_FORMAT_EXAMPLE = "无；如需格式骨架，请在 output_format 中说明。"
DEFAULT_COT_TRIGGER = "Let’s work this out in a step by step way to be sure we have the right answer."
...
def render_prompt(payload: dict[str, Any], template_path: Path) -> str:
    ...
    if constraints:
        rendered = rendered.replace(DEFAULT_COT_TRIGGER, f"### 约束条件\n{constraints}\n\n{DEFAULT_COT_TRIGGER}")
    return rendered


def main() -> None:
    parser = argparse.ArgumentParser(description="Render a zero-shot CoT prompt from JSON.")
